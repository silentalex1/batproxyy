 ...
 export default {
  ...
  if(url.pathname==='/proxy' || url.pathname.startsWith('/proxy/')){
    ...
    try{
      ...
      if(targetUrl.includes('sentry.io')||targetUrl.includes('ingest')||targetUrl.includes('cdn-cgi/rum')||targetUrl.includes('/_/_/csp_report')||targetUrl.includes('/_/_/trace')) return new Response('',{status:204, headers:{'Access-Control-Allow-Origin':'*'}});
      const r=await fetch(targetUrl,{headers:{'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0.0.0 Safari/537.36','Accept':request.headers.get('accept')||'text/html,*/*','Accept-Language':'en-US,en;q=0.9'}, cf:{cacheTtl:60, cacheEverything:true}});
      ...
      if(text!==null && ct.includes('html')){
        ...
        const inject=`<script>(function(){window.__bpBase=${JSON.stringify(base)};function p(u){try{if(!u||typeof u!=='string')return u;if(u.indexOf('data:')===0||u.indexOf('blob:')===0||u.indexOf('javascript:')===0||u.indexOf('about:')===0||u.indexOf('mailto:')===0||u.charAt(0)==='#')return u;if(u.indexOf('/proxy?url=')===0)return u;var a=new URL(u,window.__bpBase||document.baseURI);if(a.pathname==='/proxy'||a.pathname.indexOf('/proxy')===0||a.pathname==='/api/auth'||a.pathname.indexOf('/api/auth')===0||a.pathname.indexOf('/api/admin')===0||a.pathname.indexOf('/api/suggestions')===0||a.pathname.indexOf('/api/my-games')===0||a.pathname.indexOf('/api/ai')===0||a.pathname.indexOf('/api/status')===0||a.pathname.indexOf('/api/changelogs')===0||a.pathname.indexOf('/api/sites')===0||a.pathname.indexOf('/api/check-blacklist')===0||a.pathname.indexOf('/wisp')===0||a.pathname.indexOf('/uv')===0||a.pathname.indexOf('/epoxy')===0||a.pathname.indexOf('/baremux')===0||a.pathname.indexOf('/site')===0)return a.pathname+a.search+a.hash;var b=new URL(window.__bpBase||document.baseURI);if(a.origin===window.location.origin){return '/proxy?url='+encodeURIComponent(b.protocol+'//'+b.host+a.pathname+a.search+a.hash);}return '/proxy?url='+encodeURIComponent(a.href);}catch(e){return u;}}var of=window.fetch;window.fetch=function(i,n){try{var u=typeof i==='string'?i:(i&&i.url?i.url:null);if(u&&(u==='about:blank'||u==='about:srcdoc'||u.indexOf('about:blank')===0))return Promise.resolve(new Response('',{status:204})); if(u&& (u.indexOf('sentry.io')>-1||u.indexOf('cdn-cgi')>-1||u.indexOf('/_/_/')>-1)) return Promise.resolve(new Response('',{status:204})); if(typeof i==='string')return of(p(i),n);if(i&&typeof Request!=='undefined'&&i instanceof Request)return of(new Request(p(i.url),i),n);if(i&&i.url)return of(new Request(p(i.url),i),n);}catch(e){}return of(i,n);};var oo=XMLHttpRequest.prototype.open;XMLHttpRequest.prototype.open=function(){var a=[].slice.call(arguments);if(typeof a[1]==='string'){if(a[1].indexOf('sentry.io')>-1||a[1].indexOf('cdn-cgi')>-1||a[1].indexOf('/_/_/')>-1){a[1]='about:blank';}else a[1]=p(a[1]);}return oo.apply(this,a);};var osa=Element.prototype.setAttribute;Element.prototype.setAttribute=function(n,v){try{if(typeof v==='string'&&(n==='src'||n==='href'||n==='action')&&v.indexOf('data:')!==0&&v.indexOf('blob:')!==0&&v.indexOf('javascript:')!==0&&v.charAt(0)!=='#')v=p(v);}catch(e){}return osa.call(this,n,v);};window.addEventListener('unhandledrejection',function(e){e.preventDefault();},{capture:true});window.addEventListener('error',function(e){if(e&&e.message&&/OW is not defined|WebSocket|CORS|Failed to load|AngularJS|ApolloClient|PurchaseDialog|Sentry|RealTime|SignalR|cdn-cgi\\/rum|sentry\\.io/i.test(e.message))e.preventDefault();},{capture:true});(function(){var _w=console.warn,_e=console.error;console.warn=function(){var m=String(arguments[0]||'');if(/AngularJS|ApolloClient|PurchaseDialog|RealTime|SignalR|Sentry|cdn-cgi/i.test(m))return;return _w.apply(console,arguments);};console.error=function(){var m=String(arguments[0]||'');if(/AngularJS|ApolloClient|PurchaseDialog|RealTime|SignalR|Sentry|cdn-cgi|Failed to load/i.test(m))return;return _e.apply(console,arguments);};})();(function(){var OWS=window.WebSocket;if(!OWS)return;window.WebSocket=function(u,p){try{if(typeof u==='string'&&(u.indexOf('realtime.roblox.com')>-1||u.indexOf('sentry.io')>-1)){var m=new EventTarget();setTimeout(function(){m.dispatchEvent(new CloseEvent('close'));if(m.onclose)m.onclose({});},50);m.send=function(){};m.close=function(){};m.readyState=3;return m;}}catch(x){}return new OWS(u,p);};window.WebSocket.prototype=OWS.prototype;})();})();<\/script>`;
