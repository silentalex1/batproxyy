...
interface Msg { id: number; room: string; user: string; display: string; text: string; ts: number; sys?: boolean }
...
export default function Chatting() {
  ...
  useEffect(() => {
    if (!me || gate) return;
    loadLists();
    const a = setInterval(() => loadMessages(room.id), 4000);
  ...
  const send = async (e?: React.FormEvent) => {
    ...
    setText('');
    try {
      await fetch('/api/chat/messages', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ room: room.id, user: me, text: t }) });
        ...
        <div className="flex-1 flex gap-3 min-h-0 pt-3">
          ...
          <div className="flex-1 flex flex-col bg-black/55 border border-white/10 rounded-2xl backdrop-blur-md overflow-hidden min-w-0">
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1 min-h-0">
              ...
              {messages.map((m, i) => {
                ...
                if (room.kind === 'dm') {
                  return (
                    <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${mine ? 'text-white' : 'bg-white/[0.07] text-white/85'}`} style={mine ? { background: 'var(--bp-accent)' } : undefined}>
                        {renderText(m.text)}
                ...
                return (
                  <div key={m.id} className="flex gap-2.5 px-1 py-1 rounded-lg hover:bg-white/[0.02] group">
                    {!grouped ? (
                      <button onClick={() => setCard({ username: m.user })} className="w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm shrink-0 mt-0.5" style={{ background: avatarColor(m.user) }}>
                    ...
                    <div className="min-w-0 flex-1">
                      {!grouped && (
                        <p className="text-xs mb-0.5">
                          <button onClick={() => setCard({ username: m.user })} className="font-bold text-white hover:underline">{m.display || m.user}</button>
            ...
            <form onSubmit={send} className="p-3 border-t border-white/[0.06]">
              <div className="flex items-center gap-2 bg-white/[0.05] border border-white/10 rounded-full pl-5 pr-1.5 py-1.5 focus-within:border-purple-500/50 transition-all">
                <input value={text} onChange={e => setText(e.target.value)} placeholder={`Message ${room.kind === 'community' ? 'Community' : dispOf(room.label)}`} className="flex-1 bg-transparent text-white placeholder-white/30 focus:outline-none text-sm min-w-0" maxLength={500} />
          ...
          <div className="w-52 shrink-0 hidden lg:flex flex-col bg-black/55 border border-white/10 rounded-2xl backdrop-blur-md overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto px-2 pb-3 space-y-0.5">
              ...
              {online.map(o => (
                <button key={o.username} onClick={() => openDm(o.username)} className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-xl hover:bg-white/[0.05] text-left transition-all">
                  <span className="relative shrink-0">
      ...
      {card && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => setCard(null)}>
          <div className="bg-[#0b0b10]/95 border border-white/15 rounded-2xl p-7 w-full max-w-xs text-center shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-base font-bold text-white">{names[card.username] || card.username}</p>
            <p className="text-xs text-white/40 mt-0.5 mb-5">@{card.username}</p>
